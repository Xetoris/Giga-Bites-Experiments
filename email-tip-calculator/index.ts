...
const messageQueryText = `"CAFE - Transaction Receipt" after:${startSearchDate} before:${endSearchDate} label:Chase`;
...
do {
    ...
    for (const message of messageQueryResponse.messages) {
        ...
        try {
            ...
            const html = findHtmlMessage(messageBody, logger).replace(/\s+/g, '');
            const xmlMatches = TIP_XML_REGEX.exec(html);

            if(!xmlMatches) {
                logger.info('No TIP found on message.');
                continue;
            }
